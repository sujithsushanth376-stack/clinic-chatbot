...
// Initial doctor availability (which slots are ON for each doctor per weekday)
const INIT_AVAIL = {
...
HELPERS
══════════════════════════════════════════════════════════ */
function resolveDate(dateKey, weekDates) {
...
// Resolve appointment dates to real Date strings
const resolvedAppts = appointments.map(a => ({
…a,
resolvedDate: resolveDate(a.date, weekDates),
}));

// Get appointments for a specific date + doctor
                    ...
                    {dayAppts.length === 0 ? (
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                        {dayAppts.slice(0,3).map(a => (
                          <div key={a.id} style={{
                            background: STATUS_STYLE[a.status]?.bg || "#f3f4f6",
                            borderLeft: `3px solid ${STATUS_STYLE[a.status]?.dot || "#9ca3af"}`,
        ...
        {DOCTORS.map(doc => (
          <div key={doc.id} style={{ background:"#fff", borderRadius:20, padding:22, marginBottom:18, boxShadow:"0 2px 16px rgba(0,0,0,0.07)", border:"1px solid #e5e7eb" }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
              ...
              <button onClick={()=>setAvailability(p=>({...p,[doc.id]:Object.fromEntries(DAYS.slice(0,6).map(d=>[d,ALL_SLOTS]).concat([["Sun",[]]]))}))}>
              </button>
    ...
    {tab === "appointments" && (
      <div>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:18, flexWrap:"wrap" }}>
          <input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="🔍 Search patient or reason..."
            style={{ flex:1, minWidth:200, ...inputSt }} />
        ...
        <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 2px 16px rgba(0,0,0,0.07)", border:"1px solid #e5e7eb" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {filteredAppts.map((a,i) => {
                ...
                return (
                  <tr key={a.id} style={{ borderBottom:"1px solid #f1f5f9", background:i%2===0?"#fff":"#fafafa", transition:"background .1s" }}
